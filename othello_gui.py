...
from utilities import get_possible_moves, get_score
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "hcmol:d:a:b:", [
                                   "limit=", "dimension=", "agent1=", "agent2="])
    except getopt.GetoptError:
        print(
            'othello_gui.py -d <dimension> [-a <agentA> -b <agentB> -l <depth-limit> -c -o -m]')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print(
                'othello_gui.py -d <dimension> -a <agentA> [-b <agentB> -l <depth-limit> -c -o]')
            sys.exit()
